...
def prepare(recipient,revision=None):
    ...
    r=analyze(s,data);save_report(r,folder);save_archive(r,PRIVATE/'archive')
...
def main():
    ...
    if a.command=='doctor':result=connection_status(PRIVATE)
        ...
        if a.command=='render' and report['snapshot']['mode']=='live':save_archive(report,PRIVATE/'archive')
